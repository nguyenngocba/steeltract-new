...
import { classifyQueryBudget, QUERY_BUDGET_MS } from './query-budget';
...
export class RuntimeMetricsInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    const fullPath = request?.originalUrl ?? request?.url ?? endpoint;
    ...
    const stream = this.metrics.runWithRequest(requestMetrics, () =>
      next.handle().pipe(
        finalize(() => {
          const contentLength =
