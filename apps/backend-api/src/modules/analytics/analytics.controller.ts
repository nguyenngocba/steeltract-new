...
export class AnalyticsController {
  ...
  @Get()
  async getDashboard() {
    const [projects, components, purchaseOrders, inventory] = await Promise.all(
      [
    ...
    const purchaseTotal = purchaseOrders.reduce(
    ...
    return {
      ...
      aiForecast: {
        ...
        riskLevel: delayedProjects > 3 ? 'HIGH' : 'LOW',

        procurementRisk: purchaseTotal > 100000 ? 'MEDIUM' : 'LOW',
      },
    };
  }
