...
function severityFor(
  ...
  if (
    ...
    (daysUntilStockout !== null && daysUntilStockout <= 30)
  )
    return 'warning';
  return 'information';
}
...
export class DashboardMetricsService {
  ...
  private async getMaterialForecast() {
    ...
    const materialShortageForecast = items
      .map((item) => {
        ...
        const currentStock = stockByItem.has(item.id)
          ? stockByItem.get(item.id)
          : toNumber(item.quantity);
        const averageDailyUsage =
          usage.current30d > 0 ? usage.current30d / 30 : usage.total90d / 90;
        const daysUntilStockout =
          averageDailyUsage > 0
            ? Math.floor(currentStock / averageDailyUsage)
            : null;
        const recommendedFromMin = Math.max(
          0,
          toNumber(item.minimumStock) * 2 - currentStock,
        );
        const recommendedFromUsage = Math.max(
          0,
          averageDailyUsage * 30 - currentStock,
        );
        const recommendedReorderQty = Math.ceil(
          Math.max(recommendedFromMin, recommendedFromUsage),
        );
      ...
      .sort((a, b) => {
        ...
        return (
          aDays - bDays || b.recommendedReorderQty - a.recommendedReorderQty
        );
      })
      .slice(0, 12);
    ...
    const consumptionTrendRows = items.map((item) => {
      ...
      const changePercent =
        usage.previous30d > 0
          ? ratio(changeQty, usage.previous30d)
          : usage.current30d > 0
            ? 100
            : 0;
      return {
    ...
    const totalCurrentStock = Array.from(stockByItem.values()).reduce(
      (sum, qty) => sum + qty,
      0,
    );
    ...
    const trend =
      averageDailyNet < -Math.max(1, totalCurrentStock * 0.02)
        ? 'DOWN_STRONG'
        : averageDailyNet > 0
          ? 'UP'
          : 'STABLE';
      ...
      Array.from({ length: Math.min(days, 30) }, (_, index) => {
        ...
        return {
          ...
          projectedStock: Math.max(
            0,
            totalCurrentStock + averageDailyNet * (index + 1),
          ),
        };
      });
          ...
          .filter(
            (row) =>
              ...
              row.current30d >
                Math.max(
                  row.previous30d * 1.5,
                  row.current30d + row.previous30d > 0 ? 0 : 1,
                ),
          )
          .sort((a, b) => b.changePercent - a.changePercent)
          .slice(0, 8),
  ...
  private async getProductionStopRisks() {
    ...
    const { stockByItem } = await this.inventoryReadModel.getSnapshot();
      ...
      .map((order) => {
        ...
        for (const issue of order.materialIssues) {
          issuedByItem.set(
            ...
            (issuedByItem.get(issue.inventoryItemId) ?? 0) +
              Math.max(
                0,
                toNumber(issue.issuedQty) - toNumber(issue.returnedQty),
              ),
          );
        }
        ...
        const missingMaterials = (order.bom?.items ?? [])
          .map((bomItem) => {
            const requiredQty =
              toNumber(bomItem.quantity) *
              (1 + toNumber(bomItem.wastePercent) / 100) *
              Math.max(1, toNumber(order.quantity));
            ...
            const shortageQty = Math.max(
              0,
              requiredQty - issuedQty - availableQty,
            );
            return {
        ...
        const delayRiskDays =
          order.plannedEndAt && missingMaterials.length
            ? Math.max(
                1,
                Math.ceil((order.plannedEndAt.getTime() - Date.now()) / dayMs) *
                  -1 || 1,
              )
            : 0;
