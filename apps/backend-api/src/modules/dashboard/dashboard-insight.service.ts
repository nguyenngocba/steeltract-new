...
export class DashboardInsightService {
  ...
  private async getHealth(notifications: any) {
    const [inventory, production, yard, qc, suppliers, projects] =
      await Promise.all([
    ...
    const score = clampScore(
      modules.reduce((sum, module) => sum + module.score, 0) /
  ...
  private async productionHealth(): Promise<HealthModule> {
    const [delayedOrders, waitingLines] = await Promise.all([
      ...
      this.prisma.productionMaterialReservationLine
        .count({
          where: {
            status: 'SHORTAGE',
          },
    ...
    return {
      ...
      summary:
        delayedOrders > 0
          ? `${delayedOrders} lệnh chậm`
          : waitingLines > 0
            ? 'Có nguy cơ thiếu vật tư'
            : 'Bình thường',
  ...
  private async yardHealth(): Promise<HealthModule> {
    ...
    const score = clampScore(
      100 - Math.max(0, occupancy - 75) * 1.5 - overloadedZones * 10,
    );
    return {
      module: 'Yard',
      label: 'Bãi tập kết',
      ...
      score,
      summary:
        occupancy >= 90 ? `Sức chứa ${occupancy.toFixed(1)}%` : 'Bình thường',
  ...
  private async qcHealth(): Promise<HealthModule> {
    const [openNcr, failedInspections] = await Promise.all([
      this.prisma.nonConformanceReport
        .count({
  ...
  private async projectHealth(): Promise<HealthModule> {
    ...
    return {
      ...
      summary:
        delayedProjects > 0
          ? `${delayedProjects} công trình chậm`
          : 'Bình thường',
  ...
  private async getExecutiveSummary(trends: any) {
    ...
    const shortageCount = (trends?.materialShortageForecast ?? []).filter(
      (row: any) =>
        row.daysUntilStockout !== null && row.daysUntilStockout <= 7,
    ).length;
