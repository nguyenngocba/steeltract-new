...
export type CockpitTone = 'blue' | 'emerald' | 'cyan' | 'amber' | 'red' | 'purple' | 'indigo' | 'violet' | 'orange'

const toneColorMap: Record<CockpitTone, { text: string; bg: string; line: string; fill: string; note: string }> = {
...
export interface EnterpriseKpiCardProps {
  ...
  onClick?: () => void
  isLoading?: boolean
  state?: 'loading' | 'empty' | 'normal' | 'alert'
  className?: string
}
...
export function EnterpriseKpiCard({
  ...
  const isSkeleton = isLoading || state === 'loading'
  ...
  const item = toneColorMap[tone] || toneColorMap.blue
  const noteContent = note ?? subtitle ?? trendText
  const trendSeries = trend ?? trendData ?? [0, 0, 0, 0, 0, 0]
    ...
    <>
      <div className="relative z-10 flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          {noteContent ? (
            <div className={`mt-1 truncate text-[10px] font-semibold ${noteClassName ?? item.note}`}>
              {noteContent}
            </div>
          ) : null}
        </div>
      ...
      <KpiSparkline values={trendSeries} line={item.line} fill={item.fill} />
    </>
  )
