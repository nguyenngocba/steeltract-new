...
import { ModuleEmptyState, ModuleLoadingState, ModulePageHeader } from '@/shared/ui/modules'
  ...
  return (
    <EnterpriseModulePage>
      <div className="w-full min-w-0 flex-1 space-y-3">
        {breadcrumbs?.length ? (
          <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-1 text-[10px] font-medium uppercase tracking-[0.12em] text-slate-500">
            {breadcrumbs.map((item, index) => (
              <span key={`${item}-${index}`} className={index === breadcrumbs.length - 1 ? 'text-cyan-400' : undefined}>
                {index ? <span className="mr-1 text-slate-700">/</span> : null}
                {item}
              </span>
            ))}
          </nav>
        ) : null}
        <ModulePageHeader eyebrow={eyebrow} title={title} description={description} action={actions} />
        {tabs?.length ? (
          <nav className="overflow-auto rounded-2xl border border-cyan-300/15 bg-[linear-gradient(135deg,rgba(15,35,59,0.82),rgba(18,30,60,0.58))] p-1.5 shadow-[0_18px_44px_rgba(0,0,0,0.2),inset_0_1px_0_rgba(255,255,255,0.055)] backdrop-blur-xl">
            <div className="flex min-w-max gap-1">
              {tabs.map((tab) => {
                const className = `rounded-xl px-4 py-2.5 text-sm font-semibold transition ${
                  activeTab === tab.id
                ...
                return tab.path ? (
                  <Link key={tab.id} to={tab.path} className={className}>{tab.label}</Link>
                ) : (
