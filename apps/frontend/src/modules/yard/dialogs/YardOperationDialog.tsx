...
export function YardOperationDialog({
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
    <section className="flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded border border-cyan-900 bg-[#061321] shadow-2xl shadow-cyan-950/40">
      ...
      <div className="grid gap-0 overflow-y-auto lg:grid-cols-[1fr_300px]">
        <div className="p-3">
          ...
          <div className="grid gap-1 md:grid-cols-2">
            ...
            {mode !== 'outbound' && <>
              <label className="text-xs text-slate-455">Zone đích
                <select value={zoneId} onChange={(event) => { setZoneId(event.target.value); setSlotId('') }} className={`${input} mt-1`}><option value="">Chọn zone</option>{zones.map((item) => <option value={item.id} key={item.id}>{item.code} · {item.name}</option>)}</select>
              </label>
              <label className="text-xs text-slate-455">Vị trí đích
                <select value={slotId} onChange={(event) => setSlotId(event.target.value)} className={`${input} mt-1`}><option value="">Chọn slot còn tầng trống</option>{availableSlots.map((item) => <option value={item.id} key={item.id}>{item.code} · tầng kế tiếp L{item.currentStackLevel + 1}/{item.maxStackLevel}</option>)}</select>
                {zoneId && !availableSlots.length ? <span className="mt-1 block text-[11px] text-amber-305">Zone này không còn slot có tầng trống.</span> : null}
              </label>
            </>}
            {mode === 'inbound' && <>
          ...
          {error && <p className="mt-2 rounded border border-red-900 bg-red-950/30 p-2 text-xs text-red-300">{error instanceof Error ? error.message : 'Không thể hoàn tất thao tác.'}</p>}
        </div>
      ...
      <footer className="flex justify-end gap-1.5 border-t border-slate-800 bg-[#040d18]/70 px-4 py-3">
        <button onClick={onClose} className="rounded border border-slate-700 px-4 py-1.5 text-xs text-slate-350 bg-white/[0.02]">Hủy</button>
        <button disabled={pending || inboundQuantityInvalid || (mode !== 'outbound' && !slotId) || (mode === 'outbound' && !placementId)} onClick={submit} className="rounded bg-blue-600 px-5 py-1.5 text-xs font-semibold text-white disabled:opacity-50 transition hover:bg-blue-700">{pending ? 'Đang xử lý...' : labels[mode][3]}</button>
      </footer>
    </section>
  </div>
}
