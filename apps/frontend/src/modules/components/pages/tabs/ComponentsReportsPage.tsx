...
import { CockpitChartCard, CockpitKpiCard, COCKPIT_HEIGHTS } from '../../../../shared/ui/cockpit'
...
export function ComponentsReportsPage() {
  ...
  const activityValues = dashboardData?.payload?.timelineActions?.map((item) => item.count) ?? historyRows.map((_, index) => index + 1)
    ...
    <ComponentsWorkspace>
      <div className="w-full min-w-0 flex-1 space-y-1">
        <div className="grid grid-cols-1 gap-1 md:grid-cols-3 xl:grid-cols-6">
          <CockpitKpiCard title="Tổng cấu kiện" value={formatQuantity(total, 0)} note="Lifecycle catalog" state="normal" tone="blue" />
          <CockpitKpiCard title="Đang gia công" value={formatQuantity(producing, 0)} note="Cut / Weld / Paint" state="normal" tone="cyan" />
          <CockpitKpiCard title="Trong kho" value={formatQuantity(stock, 0)} note="STOCK" state="normal" tone="amber" />
          <CockpitKpiCard title="Ready to ship" value={formatQuantity(ready, 0)} note="READY" state="normal" tone="emerald" />
          <CockpitKpiCard title="Đã xuất bãi" value={formatQuantity(shipped, 0)} note="SHIPPED" state="normal" tone="purple" />
          <CockpitKpiCard title="Thay đổi" value={formatQuantity(historyRows.length, 0)} note="History rows" state="normal" tone="red" />
        </div>

        <div className="grid gap-1 xl:grid-cols-12">
          <CockpitChartCard title="Phân bổ lifecycle" subtitle="Dashboard payload" className={`${COCKPIT_HEIGHTS.CHART_LG} xl:col-span-4`}>
          ...
          <CockpitChartCard title="Biến động lifecycle" subtitle="Timeline actions/history" className={`${COCKPIT_HEIGHTS.CHART_LG} xl:col-span-4`}>
            {activityValues.length ? <ComponentsMiniBars values={activityValues.map((value) => value || 1)} /> : (
          ...
          <CockpitChartCard title="Cấu kiện gần đây" subtitle="Overview rows" className={`${COCKPIT_HEIGHTS.CHART_LG} xl:col-span-4`}>
