...
export function YardOperationsPage() {
  ...
  return (
    ...
    >
      <OperationalWorkspaceHero
        eyebrow="yard logistics / digital twin"
        title="Yard Logistics Command"
        description="Coordinate-based placement, congestion, crane activity, movement pulse and snapshot-driven logistics visualization."
        metrics={[
          {
            label: 'Slots',
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        <OpsLane
          ...
          value={congestionZones.length}
          detail={congestionZones[0]?.code ?? 'clear'}
          tone={
            congestionZones.length > 0 ? 'warning' : 'success'
          }
        />
        <OpsLane
          title="Movement pulse"
          value={movements.length}
          detail={movements[0]?.itemCode ?? 'no recent moves'}
          tone="info"
        />
        <OpsLane
          title="Crane constraints"
          value={craneIssues.length}
          detail={craneIssues[0]?.code ?? 'available'}
          tone={
            craneIssues.length > 0 ? 'danger' : 'neutral'
          }
        />
        <OpsLane
          title="Stacked slots"
        ...
        <OpsLane
          ...
          value={`${Math.round(
            occupancyPercent(
              metricsQuery.data?.occupancyRate ?? 0,
            ),
          )}%`}
          detail="snapshot-driven"
          tone="info"
        />
      </div>
      <Suspense fallback={<LoadingState label="Loading yard renderer" />}>
          ...
          {congestionZones.slice(0, 6).map((zone) => (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <StatusBadge tone="warning">
                  {Math.round(
                    occupancyPercent(zone.occupancyRate),
                  )}
                  %
                </StatusBadge>
              </div>
