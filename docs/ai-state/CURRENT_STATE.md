...
On 2026-08-03, a real authenticated REST fixture completed the canonical chain
from Material Master and MAIN receipt through Production, physical QC, Finished
Goods, Yard, Dispatch, Delivery and Installation. Runtime/database evidence
proved two materials, two definitions/requirements, one PO, exactly two
ComponentInstances, one QC PASS, one QC FAIL/NCR, one Yard placement and one
DispatchItem. Inventory quantities reconciled without loss.

One workflow blocker was fixed: Yard placement now atomically transitions an
eligible physical instance to `IN_YARD`. Remaining P0 findings are active Yard
placement after delivery, projection `INT4` overflow from epoch-millisecond
aggregate versions, Inventory idempotency payload mismatch replay and Projects
pagination string coercion. Reverse physical dismantle/project surplus/vendor
return contracts and browser chart certification remain incomplete.

Report:
`docs/audits/system-e2e1-full-business-workflow-certification.md`.

## SYSTEM.INTEGRITY.2 - V1 Runtime Certification & Canonical Freeze Audit

Status: **AUDIT COMPLETE - NOT V1 FREEZE READY**

On 2026-08-03, performed the SYSTEM.INTEGRITY.2 certification audit. No
frontend/backend source, Prisma schema, migration, staging or commit action was
performed. Runtime DB evidence shows canonical foundation data exists
(14 Projects, 25 ProjectComponentRequirements, 35 ComponentInstances, 2
Finished Goods eligible instances), but the current DB has 0 active Yard
placements, 0 DispatchItems and 0 installed ComponentInstances, so the full
Finished Goods -> Yard -> Dispatch -> Delivery -> Installation chain cannot be
certified without a disposable `SYSTEM-INTEGRITY2-*` fixture.

Freeze blockers remain: legacy inventory/material movement endpoints without
canonical `PermissionsGuard`, dashboard/read-model surfaces still deriving
physical component stock from `Component.status` / `STOCK`, and Projects
execution still marking downstream dispatch as non-canonical. Estimated
engineering completion is 88%; V1 freeze readiness is 72%.
...
On 2026-08-03, canonicalized Logistics dispatch and delivery around physical
`ComponentInstance` identity. Dispatch suggestions now read only
`ComponentInstance.state = IN_YARD` rows with active Yard placement
(`removedAt = null`) and no active dispatch. New component dispatch creation
requires `componentInstanceId`; legacy component-definition `componentId`
creation is rejected. Departure transitions physical instances to
`IN_TRANSIT`, delivery receive transitions them to `DELIVERED`, and completion
records `ComponentInstance.installedAt`.

Logistics no longer updates `Component.status = DELIVERED`, and the shipment
command domain now uses `componentInstanceId` for component lines. The
Logistics UI create drawer, tables, detail drawer and dashboard analytics were
rewired to physical instance/Yard placement data and no longer render fake
component-status/top-project chart values.
...
On 2026-08-03, converged the standalone QC module with the Components/QC
canonical physical workspace. `QcPage.tsx` no longer renders its previous
inspection-centric cockpit logic, KPI calculations from `runtime.inspections`
or fake CAPA/log/report charts. It now wraps and renders the shared
`CanonicalPhysicalQcWorkspace`, which consumes
`GET /components/foundation/instances?qcScope=true`.

All QC routes still exist, but active content now uses the same
ComponentInstance -> FINAL Inspection -> Checklist -> Disposition -> NCR ->
Timeline read model as Components/QC. No schema, migration, Logistics, Yard,
Production lifecycle, Finished Goods eligibility or ComponentInstance state
...
On 2026-08-03, converted the Components internal QC workspace to the
canonical physical quality-control source. `ComponentsInternalQcPage.tsx` no
longer derives QC result, area or KPI from legacy `Component.status`. It now
uses `GET /components/foundation/instances?qcScope=true`, which returns
physical `ComponentInstance` rows plus server-side summary counts for Waiting
QC, Passed, Failed, Rework, Use-As-Is and Scrap.

The component foundation instance read model now includes final inspection
lineage, FINAL checklist items/results, NCRs and instance timeline. The UI
renders a compact physical QC table and a right-side `ModuleDetailDrawer` with
Overview, Inspection, Checklist, NCR, Disposition and History tabs. Decision
controls are intentionally locked unless an authoritative FINAL checklist is
complete and routed through the QC command contract.
...
Status: **IMPLEMENTED - MIGRATION DEPLOYED, TEST/BUILD PASS**

On 2026-08-02, implemented the physical Logistics schema foundation required
by the LOGISTICS.2 gate. `ComponentInstanceState` now includes `IN_YARD`,
`IN_TRANSIT`, and `DELIVERED`. `DispatchItem` now has nullable
`componentInstanceId` with an optional relation to `ComponentInstance` and a
non-unique index. `DispatchItem.componentId` remains unchanged for legacy
history/readability.

Migration `20260730100000_physical_logistics_schema_foundation` was reviewed
as additive only, backed up, deployed locally, and did not backfill or
fabricate logistics history. Runtime read-only evidence after migration:
24 `PLANNED`, 3 `IN_PRODUCTION`, 4 `PRODUCED_WAITING_QC`, 2 `QC_PASSED`, and
2 `QC_FAILED` ComponentInstances; 0 DispatchItems; 0 Yard placements.
Finished Goods eligibility remains limited to `QC_PASSED` and `USE_AS_IS`;
`IN_TRANSIT` and `DELIVERED` do not appear as currently available finished
goods.

Report:
`docs/audits/logistics2a-physical-logistics-schema-foundation-report.md`.

## PATCH UI.PRODUCTION.COCKPIT.1A – Restore Production Orders as Primary Workspace & Pagination

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-30, completed patch restoring "Lệnh sản xuất" table and Search/Filter toolbar to the top of `ProductionCockpitPage.tsx` directly below Page Header & KPI strip. Restored `<InventoryPagination>` below the main table (server pagination), audited project name rendering, and moved analytical charts to support section below table.

Verification: `pnpm -C apps/frontend test` (Pass), `tsc --noEmit` (Pass), `pnpm -C apps/frontend build` (Pass), `pnpm -C apps/backend-api build` (Pass), `git diff --check` (Pass).

## SPRINT UI.PRODUCTION.COCKPIT.1 – Production Overview Layout Density & Work Order Table Redesign

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-30, completed sprint reorganizing `ProductionCockpitPage.tsx` overview layout into a 4-level information hierarchy (Top 6 KPI Cards, Row 1 8-col Stage Workload & 4-col Status Distribution, Row 2 6-col Material Readiness & 6-col Attention POs, Row 3 12-col Production Activity Logs, Row 4 12-col High-Density "Lệnh sản xuất" Operational Table + 96vw Level 2 Workspace Modal).

Verification: `pnpm -C apps/frontend test` (Pass), `tsc --noEmit` (Pass), `pnpm -C apps/frontend build` (Pass), `pnpm -C apps/backend-api build` (Pass), `git diff --check` (Pass).

## Production Cockpit Overview – Production Orders Table Title Update

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-30, updated the Production Orders table header title in `ProductionCockpitPage.tsx` from `Top {N} lệnh sản xuất` / `Danh sách lệnh sản xuất` to **`Lệnh sản xuất`** in the "Tổng quan sản xuất" tab.

Verification: `pnpm -C apps/frontend test` (Pass), `tsc --noEmit` (Pass), `pnpm -C apps/frontend build` (Pass), `pnpm -C apps/backend-api build` (Pass), `git diff --check` (Pass).

## SPRINT UI.COMPONENTS.FG.1 – Finished Goods Warehouse Enterprise UI Redesign

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-30, completed sprint redesigning Finished Goods Warehouse (`ComponentsStockPage.tsx`) with a 3-level information architecture (Level 1 8-column compact operational table, Level 2 `w-[96vw] max-w-[1720px] h-[88vh]` workspace modal, Level 3 `64vw` right-sliding Instance Detail Workspace with 5 tabs), 4 KPI cards, and strict physical `ComponentInstance` finished-goods semantics.

Verification: `pnpm -C apps/frontend test` (Pass), `tsc --noEmit` (Pass), `pnpm -C apps/frontend build` (Pass), `pnpm -C apps/backend-api build` (Pass), `git diff --check` (Pass).

## SPRINT UI.COMPONENTS.DEFINITIONS.2 – Component Detail Sliding Workspace

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-30, completed sprint redesigning Component Detail into a right-sliding detail workspace (`w-[64vw] max-w-[1280px] min-w-[820px] 100vh`) with backdrop blur, compact identity header, 6-card KPI strip, 6 horizontal tabs (`Tổng quan`, `BOM`, `Nhu cầu`, `Sản xuất`, `Instances`, `Lịch sử`), and full canonical data integration.

Verification: `pnpm -C apps/frontend test` (Pass), `tsc --noEmit` (Pass), `pnpm -C apps/frontend build` (Pass), `pnpm -C apps/backend-api build` (Pass), `git diff --check` (Pass).

## PATCH UI.COMPONENTS.DEFINITIONS.1C – Fix Actual "Chi tiết" Component Inspector Size

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-30, resolved root cause of large detail modal by updating `ModuleDetailDrawer` to remove prepended `w-screen` and adding `maxHeightClass` support. Updated Level 3 Component Detail inspector in `ComponentsListPage.tsx` to a centered `600px` max width (`w-[min(600px,calc(100vw-32px))]`), bounded max height to `72vh`, and compacted all inner content (header, metric strip, 2-column info grid, RequirementTable, and Production Lineage).

Verification: `pnpm -C apps/frontend test` (Pass), `tsc --noEmit` (Pass), `pnpm -C apps/frontend build` (Pass), `git diff --check` (Pass).

## PATCH UI.COMPONENTS.DEFINITIONS.1B – Expanded Workspace & Compact Component Detail

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-30, completed patch sprint for `ComponentsListPage.tsx`. Expanded Level 2 "Xem tất cả" modal to a near-full workspace (`w-[96vw] max-w-[1720px] h-[88vh]`) with 12-column width redistribution. Compacted Level 3 Component detail drawer to `w-[min(660px,calc(100vw-40px))]` and `max-h-[76vh]`, replacing 4 KPI cards with a 3-column metric strip and dense 6-item info grid.

Verification: `pnpm -C apps/frontend test` (Pass), `tsc --noEmit` (Pass), `pnpm -C apps/frontend build` (Pass), `git diff --check` (Pass).

## PATCH UI.COMPONENTS.DEFINITIONS.1A – Expanded Actions & Compact Detail Popup

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-30, completed patch sprint for `ComponentsListPage.tsx`. Added real API delete action (`useDeleteComponent`) with Delete Confirmation Dialog and trash icon in Level 2 "Xem tất cả" workspace. Compacted Level 3 Component detail drawer width to `780px` (`w-[min(780px,calc(100vw-48px))]`), bounded max-height to `82vh`, and ensured single body scroll owner.

Verification: `pnpm -C apps/frontend test` (Pass), `tsc --noEmit` (Pass), `pnpm -C apps/frontend build` (Pass), `pnpm -C apps/backend-api build` (Pass), `git diff --check` (Pass).

## SPRINT UI.COMPONENTS.DEFINITIONS.1 – Component Definition List Density & Detail Redesign

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-30, redesigned Component Definition workspace (`ComponentsListPage.tsx`). Implemented 3-level information architecture (Level 1 8-column operational table, Level 2 12-column modal view via "Xem tất cả", Level 3 Engineering detail drawer), single-line cell truncation (`truncate`), compact typography (`font-medium`), tabular num alignment, and responsive layout. Enforced strict domain semantics (Component = engineering definition only).

Verification: `pnpm -C apps/frontend test` (Pass), `tsc --noEmit` (Pass), `pnpm -C apps/frontend build` (Pass), `pnpm -C apps/backend-api build` (Pass), `git diff --check` (Pass).

## SPRINT UI.COMPONENTS.MATERIALS.1 – Production Material Warehouse Table Redesign

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-30, redesigned the Production Material Warehouse table (`ComponentsMaterialStockPage.tsx`). Implemented 2-level information architecture (Level 1 9-column operational table, Level 2 13-column modal view via "Xem tất cả"), single-line cell truncation, compact typography (`font-medium`), tabular num alignment, and responsive layout.
...
On 2026-07-28, performed certification/stability review for the canonical
Components -> Production -> QC -> Finished Goods workflow implemented through
DOMAIN.2 -> DOMAIN.5F.4.

Certification result:

- service-level canonical workflow tests pass
- Prisma schema is valid
- database migrations are up to date
- backend route registration includes the canonical Components foundation,
  Production command, ComponentInstanceExecution, QC final command and Finished
  Goods endpoints
- no P0 data corruption, wrong physical identity, duplicate instance
  generation or incorrect Finished Goods eligibility issue was found
- authenticated API/browser smoke is **blocked** because the Nest runtime failed
  during startup with `PrismaClientInitializationError: Can't reach database
...
Verification passed: Prisma validate, Prisma migrate status, targeted canonical
tests, full backend tests, frontend tests, backend build and frontend build.

Next required action: fix runtime DB connectivity for Nest startup, then rerun
DOMAIN5G authenticated API/browser smoke with a fresh `DOMAIN5G-*` fixture.

## COMPONENT DOMAIN.5F.4 – Canonical QC Physical Instance UI

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-28, converted the QC final manufactured-component inspection UI to
operate on canonical physical `ComponentInstance` identity.

Implemented:

- `/qc/final` now sources its waiting queue from
  `GET /components/foundation/instances?state=PRODUCED_WAITING_QC`
- each final QC row represents one physical ComponentInstance, not a Component
  definition or aggregate ProductionOrder quantity
- final QC detail modal keeps physical identity visible: instance code,
  Component definition, Project, ProjectComponentRequirement, ProductionOrder,
  production completion date and operation evidence
- final PASS/FAIL uses canonical QC command endpoints backed by
  `QcCommandService`
- PASS transitions the exact instance to `QC_PASSED`
- FAIL transitions the exact instance to `QC_FAILED` and can create canonical
  NCR with `componentInstanceId`
- command routes for NCR rework, scrap and use-as-is dispositions are exposed
  for the next NCR detail UI slice
- legacy inbound, in-process, NCR, CAPA, report and log workflows remain
...
Verification passed: Prisma validate, targeted QC command test, targeted QC
controller test, full backend tests, frontend tests, backend build, frontend
...
On 2026-07-28, integrated the Production UI with the canonical
ProjectComponentRequirement -> ProductionOrder -> ComponentInstance workflow.

Implemented:

- Manufacturing Order creation now starts from
  `ProjectComponentRequirement`, not arbitrary Component + legacy Production
  BOM selection
- create modal shows Project, requirement, required quantity, allocated
  Production Order quantity, remaining quantity and Engineering basis readiness
- canonical create path calls `POST /production/commands/orders`
- canonical release action calls `POST /production/commands/orders/:id/release`
  and uses materialized BOM routing to create WorkOrders
- Production Order drawer shows canonical physical `ComponentInstance` rows
  from `/components/foundation/instances?productionOrderId=...`
- physical state labels are localized and QC handoff is visible without
  executing QC decisions in Production
- legacy manual `Tạo cấu kiện từ MO` action was removed from the main drawer
  semantics
- foundation read API now includes requirement allocation and instance
  execution lineage
...
Verification passed: Prisma validate, production command targeted tests,
component domain foundation targeted tests, frontend tests, backend build,
frontend build and `git diff --check`.

## COMPONENT DOMAIN.5F.2 – Canonical Component Definition UI

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-28, converted Components `/components/list` to represent canonical
**Hồ sơ cấu kiện** instead of a mixed physical/production component list.

Implemented:

- read-model fields for `Component.lifecycleState`, engineering status,
  current revision, BOM state and ProjectComponentRequirement summaries
- engineering/planning KPI strip:
  `Tổng hồ sơ`, `Hồ sơ nháp`, `Đã phát hành sản xuất`,
  `Tổng nhu cầu cấu kiện`, `Chưa có BOM`
- engineering filters instead of physical status filters
- main table columns aligned to engineering definitions and Project
  requirements
- detail drawer requirement subtable with required, allocated and remaining
  quantities
- test expectation that legacy `Component.status=STOCK` does not become
  `Tồn kho` on the definition page

Deliverable:

- `docs/audits/component-domain5f2-component-definition-ui-report.md`

Verification passed: read-model repository test, domain foundation targeted
tests, full backend tests, frontend tests, backend build, frontend build and
`git diff --check`.

## COMPONENT DOMAIN.5F.1 – Canonical Finished Goods UI

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-28, implemented the first UI integration slice after DOMAIN.5F.
Components `/components/stock` now represents canonical **Cấu kiện thành phẩm**
from `GET /components/instances/finished-goods`, not legacy Component
definition stock.

Implemented:

- frontend Finished Goods contract/API/query hook
- Components stock route relabeled to `Cấu kiện thành phẩm`
- physical `ComponentInstance` table with Component definition, Project,
  Production Order, production completion date, QC evidence, physical state
  and non-fabricated location display
- KPI strip from backend Finished Goods summary only
- server-backed filters for instance code, Project and Component definition
- detail drawer showing canonical lineage
- backend read response summary for the existing Finished Goods endpoint

Removed from the page semantics:

- `Component.status` READY/STOCK/SHIPPED/DELIVERED/INSTALLED as physical
  Finished Goods evidence
- `COUNT(Component)` as physical stock
- frontend Yard/Production/BOM/Audit reconstruction for Finished Goods

Deliverable:

- `docs/audits/component-domain5f1-finished-goods-ui-report.md`

Verification passed: backend targeted tests, full backend tests, frontend
tests, backend build, frontend build and `git diff --check`.

## COMPONENT DOMAIN.5F – Canonical Components / Production / QC UI Integration Audit

Status: **AUDIT COMPLETE - NO CODE IMPLEMENTATION**

On 2026-07-28, audited the current Components, Production and QC frontend
against the canonical backend workflow now available through DOMAIN.2 ->
DOMAIN.5E. The audit confirmed the backend can represent engineering
definitions, project requirements, physical component instances, instance
execution evidence, instance-level QC and finished-goods eligibility, but the
frontend is only partially aligned.

Primary finding:

- Components create is mostly canonical, but list/dashboard/stock screens still
  mix engineering definitions with physical inventory.
- Production UI still works mainly at aggregate ProductionOrder/stage quantity
  level and does not expose generated physical instances or
  `ComponentInstanceExecution` assign/start/complete actions.
- QC UI still queues and approves inspections by ProductionOrder/Component
  instead of targeting final QC by exact `componentInstanceId`.
- Existing Component Inventory-style screens must migrate to
...
No source code, schema, migration, staging or commit action was performed.

## COMPONENT DOMAIN.5D – ComponentInstanceExecution Schema Foundation

Status: **IMPLEMENTED - MIGRATION/TEST/BUILD/RUNTIME PASS**

On 2026-07-28, implemented the approved DOMAIN.5C physical production
traceability foundation. `ProductionExecution` remains the batch/run header;
new `ComponentInstanceExecution` rows now represent one physical
`ComponentInstance` participating in one WorkOrder operation/run.

Implemented:

- `ComponentInstanceExecutionStatus` enum:
  `ASSIGNED`, `RUNNING`, `COMPLETED`, `CANCELLED`
- `ComponentInstanceExecution` model/table
- relations to `ComponentInstance`, `WorkOrder`, `ProductionExecution`
- uniqueness on `componentInstanceId + productionExecutionId`
- minimum repository/service/API command foundation for assign/start/complete/
  cancel/history
- runtime smoke using DOMAIN4 PO-A with temporary execution run and cleanup

DOMAIN.5D intentionally does not mutate `ComponentInstance.state`, does not
create QC/Inventory/Yard/Fini­shed Goods side effects, and does not backfill
legacy ProductionExecution history.

Deliverable:

- `docs/audits/component-domain5d-instance-execution-foundation-report.md`

Verification: Prisma validate/generate/migrate status (Pass), targeted tests
(5 suites / 31 tests Pass), full backend tests (79 suites / 244 tests Pass),
backend build (Pass), frontend build (Pass with existing Vite chunk-size
warning), runtime smoke (Pass).

## COMPONENT DOMAIN.5C – Production Instance Execution Granularity Audit & Design

Status: **AUDIT / ARCHITECTURE DESIGN COMPLETE - NO IMPLEMENTATION**

On 2026-07-28, completed the DOMAIN.5C audit and design for the permanent
bridge from Production execution to physical `ComponentInstance` identity.

Finding:

- Current Production start persists order/work-order/execution-run evidence.
- Current completion persists aggregate completed quantity and optional
  WorkOrder/executionRun IDs.
- Current schema does not persist which physical ComponentInstances started or
  completed each operation.
- Partial production such as `001-005=CUTTING`, `006-010=WELDING`,
  `015-017=PRODUCED_WAITING_QC` cannot be represented truthfully.

Recommended canonical model:

- add a future `ComponentInstanceExecution` entity
- keep `ProductionExecution` as the batch/run header
- store one physical execution line per ComponentInstance per WorkOrder /
  operation / execution run
- transition `ComponentInstance` state only from explicit physical execution
  evidence
...
Status: **IMPLEMENTED - DOMAIN.5 BLOCKED BY PRODUCTION INSTANCE GRANULARITY**

On 2026-07-28, deployed the additive DOMAIN.5B enum migration after the backup
gate. `ComponentInstanceState.IN_PRODUCTION` now exists in Prisma and
PostgreSQL, and existing `ComponentInstance` row counts remained unchanged.

Migration:

- `20260728090000_component_domain5b_instance_in_production_state`
- SQL: `ALTER TYPE "ComponentInstanceState" ADD VALUE IF NOT EXISTS 'IN_PRODUCTION'`
- Backup: `/tmp/steeltrack-domain5b-before-20260728.dump`
- Pre/post `ComponentInstance` count: `16`
- Pre/post grouped states: all rows remained `PLANNED`

DOMAIN.5 physical transitions were intentionally not implemented. Current
Production start/completion commands operate at Production Order, WorkOrder,
Execution Run and aggregate completed-quantity granularity. They do not
identify which physical `ComponentInstance` IDs, serial sequences or serial
ranges started/completed. Updating all instances would fabricate history for
partial production.

Next gate: define the approved Production instance execution granularity before
implementing `PLANNED -> IN_PRODUCTION -> PRODUCED_WAITING_QC` transitions.

## COMPONENT DOMAIN.5A – QC Physical Instance Lineage Foundation

Status: **IMPLEMENTED - MIGRATION/TEST/BUILD/RUNTIME PASS**

On 2026-07-27, implemented the additive QC physical lineage foundation required
before Finished Goods gating. `QcInspection`, `NonConformanceReport` and
`QcInspectionSnapshot` now have nullable `componentInstanceId` fields,
relations and indexes. `ComponentInstance` now exposes inspection/NCR
relations.

The migration
`20260727224000_component_domain5a_qc_instance_lineage` was deployed after a
verified PostgreSQL backup
`/tmp/steeltrack-domain5a-before-20260727.dump`. Pre/post row counts for
`QcInspection`, `NonConformanceReport`, `QcInspectionSnapshot` and
`ComponentInstance` were unchanged.

QC service foundation now accepts `componentInstanceId`, validates instance /
component / production order / project lineage, derives context fields from
the physical instance when omitted, preserves NCR instance lineage, and carries
`componentInstanceId` through canonical QC/NCR/disposition events and
inspection snapshots.

Runtime smoke created one instance-level QC inspection and one NCR against
controlled DOMAIN4 instance `cms31uquo001epvk716nvtrja`. It created no
ComponentInstances, InventoryTransactions, Yard placements or Finished Goods
classification.

Verification: Prisma validate/generate/migrate status (Pass), targeted
QC/snapshot tests (Pass, 4 suites / 13 tests), backend tests (Pass, 78 suites /
234 tests), backend build (Pass), frontend build (Pass with existing Vite
chunk-size warning).

## COMPONENT DOMAIN.5 – Production Completion, QC & Finished Goods Gate

Status: **BLOCKED BY COMPONENTINSTANCE STATE MODEL GATE**

On 2026-07-27, audited the Production completion, QC and Finished Goods gate
for the canonical ComponentInstance workflow. Implementation was intentionally
blocked before code changes at audit time because the QC schema could not
structurally reference physical `ComponentInstance` rows.

Current QC models only support `productionOrderId`, `productionStageId`,
legacy `componentId` and `projectId`. `QcInspection` and
`NonConformanceReport` do not have `componentInstanceId`, so QC PASS/FAILED/
REWORK/SCRAP cannot be proven for one physical manufactured component inside a
Production Order batch. Using metadata would not satisfy canonical lineage or
future Finished Goods/Yard/Logistics queries.

Created `docs/audits/component-domain5-qc-finished-goods-report.md` with the
...
- relations from `ComponentInstance` to inspections/NCRs
- nullable indexes on those new fields

No code, schema, migration, tests or runtime writes were performed in DOMAIN.5.

DOMAIN.5A has now closed this schema gate. DOMAIN.5 may resume with production
completion, QC PASS/rework/scrap and Finished Goods eligibility.

## COMPONENT DOMAIN.4 – Production Integration & Physical Instance Creation

Status: **IMPLEMENTED - TEST/BUILD/RUNTIME SMOKE PASS**

On 2026-07-27, connected canonical ProjectComponentRequirement demand to the
Production command boundary. `POST /production/commands/orders` now accepts
optional `componentRequirementId` and validates requirement/component/project,
released Engineering basis, materialized Production BOM lineage and active
quantity allocation before creating a DRAFT Production Order. Requirement-bound
orders preserve `componentRequirementId`, `componentId`,
`componentRevisionId`, `bomDefinitionId` and `bomId`.

Production Order release is now the canonical manufacturing authorization
point for physical identity creation. Releasing a requirement-bound order
creates one `ComponentInstance` per integer Production quantity in
`PLANNED` state, with stable server-side instance numbers and lineage to
Component, Revision, BOM definition, Production Order, Requirement and Project.
Release replay with the same idempotency key does not create duplicates.

Runtime smoke namespace `DOMAIN4-1785146027125` created a requirement quantity
20, PO-A quantity 8 and PO-B quantity 7. Draft PO-A had 0 instances; releasing
PO-A created 8 planned instances; replay did not increase the count; releasing
PO-B brought the requirement total to 15; PO-C quantity 6 was rejected as
over-allocation. DB verification confirmed 15 planned instances, 15 instance
timeline rows, 0 QC PASS, 0 producedAt, 0 InventoryTransactions and 0 Yard
placements.

Verification: Prisma migrate status/validate/generate (Pass), targeted
Production command tests (Pass, 9 tests), backend tests (Pass, 78 suites / 230
tests), backend build (Pass), frontend build (Pass with existing Vite
chunk-size warning), runtime authenticated API/DB smoke (Pass).

## COMPONENT DOMAIN.3 – Component Create & Project Requirement Conversion

Status: **IMPLEMENTED - MIGRATION/TEST/BUILD PASS**

On 2026-07-27, converted the new Components UI create path to canonical
planning semantics. New canonical create now uses an additive
`POST /components/foundation/definition-requirements` operation that creates one
engineering `Component` in `DRAFT` lifecycle state and one
`ProjectComponentRequirement.requiredQuantity` in a single transaction. It does
not create ComponentInstances, ProductionOrders, inventory quantity, Finished
Goods or Yard stock. Component code and requirement number are backend-owned
and unique-constrained with retry.

The Prisma schema now has additive typed engineering attributes
`Component.componentType` and `Component.profile`, with an index on
`componentType`. Existing records remain nullable and are not backfilled.
Components read models prefer typed fields and requirement quantity, then fall
back to legacy `description` JSON for old records. Legacy `/components` remains
readable.

The backup gate was cleared through the approved native-client execution path:
`/tmp/steeltrack-domain3-before-20260727.dump` was created and verified with
`pg_restore --list`. Migration
`20260727223000_component_domain3_typed_definition_fields` was deployed with
`prisma migrate deploy`; Prisma migrate status reports the database schema is
up to date. Existing Component, Requirement, ComponentInstance, ProductionOrder,
BOM and BOMItem row counts were unchanged after migration, existing records
kept `componentType/profile = NULL`, and STABILITY7/B1 lineage remains readable.

Runtime smoke created controlled namespace `DOMAIN3-1785144581343` through the
real authenticated API. It produced exactly 1 Component definition in
`DRAFT`, 1 ProjectComponentRequirement with `requiredQuantity=20`, 0
ComponentInstances, 0 ProductionOrders and 0 InventoryTransactions.

Verification: Prisma migrate status/validate/generate (Pass), backend tests
(Pass, 78 suites / 227 tests), backend build (Pass), frontend build (Pass with
existing Vite chunk-size warning), `git diff --check` (Pass).

## COMPONENT DOMAIN.2 – Canonical Schema Foundation

Status: **IMPLEMENTED - MIGRATION/TEST/BUILD PASS**

On 2026-07-27, implemented the first permanent canonical Component domain
schema foundation. `Component` remains the engineering definition, while new
additive tables now represent project demand/planning and physical component
identity: `ProjectComponentRequirement`, `ComponentInstance` and
`ComponentInstanceTimeline`. `ProductionOrder` now has an optional
`componentRequirementId` lineage field for future Production conversion.

Legacy Component fields, legacy APIs, Engineering BOM B1, Production Warehouse,
Inventory, QC, Yard, Logistics, Historical Dashboard, Snapshot Engine and
frontend flows were not converted or redesigned. No ambiguous legacy Component
record was fabricated into a ComponentInstance.

Runtime smoke created controlled DOMAIN2 requirements and one PLANNED physical
instance, confirmed requirements do not create inventory or instances, confirmed
one Component can have requirements from multiple Projects, and confirmed B1
released BOM lineage remains usable.

Verification: Prisma migrate deploy/status/validate/generate (Pass), backend
tests (Pass, 78 suites / 223 tests), backend build (Pass), frontend build
(Pass with existing Vite chunk-size warnings), `git diff --check` (Pass).

## SPRINT STABILITY.7 – B1 Runtime Integration Certification

Status: **CERTIFIED - RUNTIME FIXTURE PASS**

On 2026-07-27, closed the STABILITY.5 B1 runtime integration condition with a
controlled fixture namespace `STABILITY7-1785129145020`. The certification used
the existing runtime services and created a dedicated Component, Revision,
Engineering BOM, Production Order and materialized Production BOM. It confirmed
the full path from Component `DRAFT` through Engineering Release into
Production Order BOM binding.

Key results: Production creation before Engineering Release was rejected,
invalid Engineering BOM input was rejected, R1 Engineering BOM validated and
released, Production Order creation materialized and bound the Production BOM,
materialization replay was idempotent, and a later R2 release did not alter the
existing Production Order's R1 BOM binding.

No schema, migration, frontend, Inventory workflow, Sprint C-J feature, staging
or commit action was performed. Fixture records are retained under the
namespace for audit traceability.

Verification: Prisma migrate status (Pass), Prisma validate (Pass), backend
tests (Pass, 76 suites / 215 tests), backend build (Pass), frontend build
(Pass with existing Vite chunk-size warnings), `git diff --check` (Pass).

## SPRINT STABILITY.6 – Historical Snapshot Date Normalization

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-27, resolved the STABILITY.5 Historical Snapshot `@db.Date`
timezone/date mismatch. Snapshot business dates now use a single canonical
rule: `YYYY-MM-DD` calendar dates are persisted as UTC-midnight Date values,
API business-date inputs are strict `YYYY-MM-DD`, API business-date outputs are
`YYYY-MM-DD`, and technical timestamps remain ISO strings.

Runtime evidence under both `UTC` and `Asia/Ho_Chi_Minh` confirmed that local
midnight inputs such as `2026-07-28T17:00:00.000Z` normalize to the intended
business date `2026-07-29`. A controlled `YARD/stability6_dashboard_daily`
Snapshot Engine run stored `snapshot_jobs`, `dashboard_snapshots` and
`snapshot_metadata` on `2026-07-29` with non-authoritative/stale historical
marking. Historical Dashboard controller/service/repository reads returned
`snapshotDate: "2026-07-29"`.

Verification: Prisma validate/generate/migrate status (Pass), targeted
Snapshot/Historical Dashboard tests (Pass), backend tests (Pass, 76 suites /
215 tests), backend build (Pass), frontend build (Pass), `git diff --check`
(Pass).

## SPRINT STABILITY.5 – Full System Runtime Certification

Status: **CERTIFIED WITH CONDITIONS - TEST/BUILD PASS**

On 2026-07-27, certified the current SteelTrack baseline across database,
backend runtime, authenticated module APIs, frontend route shell, background
services, Snapshot minimum workflow and regression suites. No P0 remains.

Critical APIs for Inventory, Components, Production, QC, Projects, Suppliers,
Logistics, Yard, UOM, Admin/System and Historical Snapshot returned controlled
responses with no unexpected 500s. Snapshot Engine minimum workflow seeded
controlled `INVENTORY/dashboard_daily` and `INVENTORY/inventory_balance_daily`
metadata, scheduled and completed 2 jobs, wrote 1 dashboard snapshot, 77
inventory balance snapshots and 6 INFO job logs.

Certification is conditional because Historical Snapshot date semantics show a
P1 timezone/date mismatch: current-day snapshot generation stored `@db.Date`
rows on `2026-07-25` during the Asia/Ho_Chi_Minh runtime check, while API reads
for later calendar dates returned controlled not-found/empty responses. B1
runtime E2E is also not fully certified with existing DB data because no
released revisions/BOM definitions are present.

Verification: Prisma migrate status/validate/generate (Pass), backend tests
(Pass, 75 suites / 210 tests), frontend tests (Pass, 1 file / 2 tests),
targeted backend tests (Pass, 13 suites / 63 tests), backend build (Pass),
frontend build (Pass), `git diff --check` (Pass).

## SPRINT STABILITY.4 – Historical Snapshot Corrective Migration

Status: **IMPLEMENTED - MIGRATION/TEST/BUILD PASS**

On 2026-07-27, resolved the Historical Snapshot schema drift blocker by creating
and deploying one forward-only corrective Prisma migration:
`20260727210000_historical_snapshot_corrective_schema`. The migration creates
only the missing Historical Snapshot enums/tables already defined in
`schema.prisma`: dashboard snapshots, inventory balance snapshots, monthly
rollups, snapshot jobs/logs, rebuild requests and snapshot metadata. Existing
legacy snapshot tables/data were preserved unchanged.

Runtime verification confirmed the previous `public.snapshot_metadata does not
exist` startup error no longer recurs. Authenticated `/history/*` smoke tests
return controlled empty or not-found responses instead of schema-level 500s.

Verification: new backup created at
`/tmp/steeltrack-stability4-20260727-104307.dump`, Prisma migrate
deploy/status/validate/generate (Pass), `pnpm -C apps/backend-api test` (Pass,
75 suites / 210 tests), `pnpm -C apps/backend-api build` (Pass),
`pnpm -C apps/frontend build` (Pass).

## Component Manufacturing Workflow Sprint B1 – Engineering BOM Materialization

Status: **IMPLEMENTED - TEST/BUILD PASS**

On 2026-07-27, implemented the minimum Engineering BOM to Production BOM bridge.
Released Component BOM definitions are now validated against a canonical
Engineering BOM contract and materialized into the existing Production `BOM` /
`BOMItem` tables before Production Order creation. Component-bound Production
Orders now bind to the materialized Production BOM and preserve the source
`componentId`, `componentRevisionId`, `bomDefinitionId`, and
`engineeringContentHash`.

No Inventory, QC, Yard, Logistics, Projects, Historical Dashboard, Snapshot
Engine, Warehouse Realtime, frontend UI, or third BOM model was introduced.
Prisma changes are additive and limited to Production BOM lineage.

Verification: Prisma validate/format/generate (Pass),
`pnpm -C apps/backend-api test` (Pass, 75 suites / 210 tests),
`pnpm -C apps/backend-api build` (Pass), `pnpm -C apps/frontend build` (Pass).
...
# COMPONENT DOMAIN.5E Physical Lifecycle and QC Handoff (2026-07-28)

Status: **IMPLEMENTED**

ComponentInstance physical state is now derived from
ComponentInstanceExecution evidence and final QC facts. Assignment remains
planning evidence only; `RUNNING` moves `PLANNED/REWORK` instances into
`IN_PRODUCTION`; `COMPLETED` moves an instance into `PRODUCED_WAITING_QC` only
when every WorkOrder on that ProductionOrder has completed execution evidence
for the same instance. Final QC acceptance moves produced instances to
`QC_PASSED`; final QC rejection moves them to `QC_FAILED`; NCR disposition can
move failed/waiting/rework instances to `REWORK`, `SCRAPPED` or `USE_AS_IS`.

`GET /components/instances/finished-goods` is available as an authenticated
read-only eligibility API. It requires physical state plus final QC/NCR evidence
and does not create Inventory, Yard or dashboard side effects. No schema change
or migration was introduced.

# STABILITY.DOMAIN5G.2 ProductionExecution REST Command Completion (2026-07-28)

Status: **BACKEND RUNTIME API GREEN / BROWSER SMOKE PENDING**

The missing REST command surface for ProductionExecution lifecycle transitions
is now exposed under `/production/commands/executions/...`. The controller
delegates to existing `ProductionCommandService` methods for start, pause,
resume, complete and abort, preserving aggregate transition rules,
idempotency, actor and correlation context.

Runtime DOMAIN5G.2 HTTP smoke certified Component Definition, Requirement,
Engineering Release, Production Order, PO Release/Ready/Start, active execution
guard, ProductionExecution complete, WorkOrder complete, Waiting QC, QC PASS,
...
# COMPONENTS.PRODUCTION.2 Canonical Production Workspace (2026-08-03)

Status: **IMPLEMENTED - TEST/BUILD PASS, BROWSER SMOKE PENDING**

Components Production now consumes `GET /production/read-model/cockpit` instead
of the legacy `/production` order list. The backend read model exposes canonical
ProductionOrder, ProjectComponentRequirement, Component Definition, Revision,
BOM Definition, ComponentInstance, ComponentInstanceExecution, QC and material
readiness data. The frontend table, KPI strip, `Xem tất cả` workspace and
right-side detail drawer now bind to that read model instead of deriving
physical progress from `ProductionOrder.quantity`.

No schema, lifecycle, Finished Goods, Yard, Logistics, Historical Dashboard or
Snapshot Engine code changed. Browser runtime certification remains pending.

# SYSTEM.INTEGRITY.1 SteelTrack V1 Operational Readiness Audit (2026-08-03)

Status: **AUDIT COMPLETE - NO SOURCE IMPLEMENTATION**

SteelTrack V1 operational readiness is currently assessed at **72%**. The
platform is conditionally ready for internal staging but not ready for V1
freeze. Inventory, Master Data, Components, Production, QC and Yard have strong
canonical foundations, but V1 is blocked by Logistics still writing dispatch
items by component definition identity, registered legacy write endpoints that
lack canonical RBAC permission guards, and dashboards/read models that still
derive some physical component metrics from `Component.status`.
