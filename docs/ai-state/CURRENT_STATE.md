...
- Inventory Overview monthly historical KPI trends and delta percentages use 12-point monthly snapshots computed using transaction ledger rollbacks and transaction-based existence dates (firstTransactionDate). Sparklines output real monthly trends when data age is >= 12 months (otherwise showing flat placeholder lines). Delta notes compare current month vs previous month dynamically (e.g. `▲/▼ X% với tháng trước`) with dynamic emerald/red semantic coloring (emerald for positive metric increases or alert reductions; red for positive metric decreases or alert increases).
